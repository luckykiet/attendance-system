...
import { SignupFormMutation, SignupForm } from '@/types/forms/auth';
...
const AuthSignup = () => {
  ...
  const signupSchema = z.object({
    username: z
      .string()
      .trim()
      .min(4, { message: 'srv_invalid_username_length' })
      .max(255, { message: 'srv_invalid_username_length' })
      .regex(/^\S+$/, { message: 'srv_invalid_username' }),
    email: z
      .string()
      .email({ message: 'srv_invalid_email' }),
    tin: z
      .string()
      .regex(/^[0-9]{8}$/, { message: 'srv_invalid_tin' }),
    name: z
      .string()
      .max(255, { message: 'srv_invalid_name_length' }),
    vin: z
      .string()
      .max(255, { message: 'srv_invalid_vin' })
      .optional(),
    address: z.object({
      street: z
        .string()
        .max(255, { message: 'srv_invalid_street_length' })
        .optional(),
      city: z
        .string()
        .max(255, { message: 'srv_invalid_city_length' })
        .optional(),
      zip: z
        .string()
        .max(20, { message: 'srv_invalid_zip_length' })
        .optional(),
    }),
    password: z
      .string()
      .min(8, { message: 'srv_invalid_password_length' })
      .max(255, { message: 'srv_invalid_password_length' }),
    confirmPassword: z
      .string()
      .max(255, { message: 'srv_invalid_password_length' })
  ...
  const mainUseForm = useForm<SignupForm>({
  ...
  const { data: tinData, isLoading: tinLoading, error: tinError, isFetched: tinFetched } = useQuery<AresCompany>({
    queryKey: ['ares', tin],
  ...
  const loginMutation = useMutation({
    mutationFn: (data: SignupFormMutation) => signup(data),
    onError: (error) => {
      console.log(error);
      setPostMsg(error);
    },
